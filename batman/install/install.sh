...
# -------- NEW: SSH / Management IP settings (eth-mgmt) -------------------------
# This section is intentionally optional: it only runs if eth-mgmt.network exists.
# It is asked BEFORE the node/br0 IP prompt as requested.

#if [ -f "${root}/etc/systemd/network/eth-mgmt.network" ]; then
#  # SSH IP (keep CIDR as in file)
#  line="$(grep '^Address=' ${root}/etc/systemd/network/eth-mgmt.network)"
#  line=${line#Address=}
#  ssh_ip_addr=${line%/*}             # e.g. 192.168.0.21
#  ssh_cidr_suffix=${line#*/}         # e.g. 24
#  ssh_cidr_old="/$ssh_cidr_suffix"   # e.g. /24 (not prompted by design)

#  replace "$ssh_ip_addr" "SSH IP Address (eth-mgmt)" "${root}/etc/systemd/network/eth-mgmt.network" ip

#  # SSH Gateway
#  if grep -q '^Gateway=' "${root}/etc/systemd/network/eth-mgmt.network"; then
#    line="$(grep '^Gateway=' ${root}/etc/systemd/network/eth-mgmt.network)"
#    ssh_gw=${line#Gateway=}
#    replace "$ssh_gw" "SSH Gateway (eth-mgmt)" "${root}/etc/systemd/network/eth-mgmt.network" ip
#  fi

  # SSH DNS
#  if grep -q '^DNS=' "${root}/etc/systemd/network/eth-mgmt.network"; then
#    line="$(grep '^DNS=' ${root}/etc/systemd/network/eth-mgmt.network)"
#    ssh_dns=${line#DNS=}
#    replace "$ssh_dns" "SSH DNS (eth-mgmt)" "${root}/etc/systemd/network/eth-mgmt.network" ip
#  fi
#else
#  LOG_TS; echo "Note: ${root}/etc/systemd/network/eth-mgmt.network not found; skipping SSH/management IP prompts."
#fi
...
replace "$ip_addr"  "Node IP Address"                 "${root}/etc/systemd/network/br0.network" ip
replace "$cidr_old" "Node IP suffix (CIDR, default 24)" "${root}/etc/systemd/network/br0.network" cidr
